(function(){
    var host='sensitive-data-detection-redaction-api.p.rapidapi.com',path='/scan',rapidUrl="https://rapidapi.com/precisionsolutionstech/api/sensitive-data-detection-redaction-api";
    ...
    document.getElementById('postman-download').addEventListener('click',function(){
        ...
        var coll = { info: { name: "PII Detection & Redaction API", schema: 'https://schema.getpostman.com/json/collection/v2.1.0/collection.json' }, item: [{ name: 'POST /scan', request: { method: 'POST', header: hdrs, url: fullUrl(), body: { mode: 'raw', raw: postmanRawBody() } } }] };
        var blob = new Blob([JSON.stringify(coll,null,2)], { type: 'application/json' });
        var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'pii-detection-redaction-postman-collection.json'; a.click(); URL.revokeObjectURL(a.href);
    });
